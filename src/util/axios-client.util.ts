...
export class AxiosClientUtil {
  ...
  public async get(url: string, headerOpts?: any): Promise<CommonResponseDto<any>> {
  ...
  public async post(url: string, data?: any, headerOpts?: any): Promise<any> {
    let headers = this.setHeaderData(headerOpts);
    try {
      const response = await firstValueFrom(this.httpService.post(url, data, {headers: headers}));
