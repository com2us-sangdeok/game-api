...
import {IsArray, IsNumberString, IsObject, IsOptional, IsString, IsUrl} from "class-validator";
...
export class ExtensionDto {
    ...
    @ApiProperty({ example: 'https://image01.c2x.world/equip_92053030.gif', description: 'animation url', required: false  })
    @IsOptional()
    @IsUrl()
    ...
    @ApiProperty({ example: 'https://dex.c2xnft.com/market?key=4423', description: 'market url', required: false })
    @IsOptional()
    @IsUrl()
    external_url?: string;

    @ApiProperty({ example: '', description: 'background color', required: false })
    @IsOptional()
    @IsString()
